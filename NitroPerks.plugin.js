    ...
    const config = {
        "info": {
            ...
            "authors": [
                {
                    "name": "ravgo",
                    "discord_id": "388869946513293322",
                    "github_username": "Ravgo"
                },
            ],
            "version": "1.4.0",
                ...
                getSettingsPanel() {
                    return Settings.SettingPanel.build(_ => this.saveAndUpdate(), ...[
                        ...
                        new Settings.SettingGroup("Emojis").append(
                            ...
                            new Settings.Slider("Size", "The size of the emoji in pixels. 48 is recommended.", 16, 64, this.settings.emojiSize, size=>this.settings.emojiSize = size, {markers:[16,20,32,48,64], stickToMarkers:true})
                        ),
                    ...
                    if (this.settings.emojiBypass) {
                        ...
                        Patcher.before(DiscordModules.MessageActions, "sendMessage", (_, [, msg]) => {
                            msg.validNonShortcutEmojis.forEach(emoji => {
                               ...
                               // msg.content = msg.content.replace(`<${emoji.animated ? "a" : ""}${emoji.allNamesString.replace("/~\d/g", "")}${emoji.id}>`, emoji.url + `.gif?size=${this.settings.emojiSize}`)
                                  msg.content = emoji.url.replace('?size=160', `?size=${this.settings.emojiSize}`)
                            })
                        });
